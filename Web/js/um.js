	...
	TMS_api(url,"GET","",function a(){
		if (xmlHttp.readyState==4 && xmlHttp.status==200){
			...
			if(resp.code==200){	
				...
				if(usrlist.length==0)page_num=0;
...
function save_userInfo(url){
	...
	else if(usrinfo.type=='local' && usrinfo.passwd==""){
		alerm("用户密码不能为空");
		$("#usr_passwd").focus();
	}
	else{		
	...
	if(ldapconf.host==""){
		...
		TMS_api(url,"POST",body,function a(){
			if (xmlHttp.readyState==4 && xmlHttp.status==200){
				...
				if(resp.code==200){
					alerm("LDAP配置保存成功");
					CloseForm('#form_ldap_conf','#overlay');
				}
				else alerm(resp.message);
			}
		});
...
$(document).ready(function(){ 
			...
			TMS_api(url,"GET","",function a(){
				if (xmlHttp.readyState==4 && xmlHttp.status==200){
					...
					if(resp.code==200){	
						...
						$("#usr_mail").val(tr.children().eq(4).text());
						$("#usr_mobile").val(tr.children().eq(5).text());
						$("#usr_type").val(tr.children().eq(6).text());
	...
	$("body").click(function b(e){
		...
		if(v_class!="tbody_usrlist"){
			...
			tr_selected.children().css("background-color","#E3F1F7");	
		}
	});
